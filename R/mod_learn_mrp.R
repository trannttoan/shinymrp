    ...
    fluidRow(
      column(width = 6, offset = 3,
        tags$div(
          ...
          withMathJax("* \\(\\alpha_{\\rm a}^{\\rm age}\\) is the effect of subject \\(i\\)'s age on the probability of having a positive response."), tags$br(),
          ...
          withMathJax("To generalize results from this model to a national or subgroup estimate, we obtain the population cells in the contigency table of sex, age, race/ethnicity, education, and state and weight the model predictions by the population frequency of the poststratification cells \\(N_j\\)'s. Suppose the cell-wise estimate based on model \\eqref{mrp-1} is \\(\\theta_j\\) in cell \\(j\\), the MRP estimate can be expressed as:"),
          ...
          withMathJax("We use a Bayesian framework to account for unknown sensitivity and specificity and apply MRP to COVID testing records for population representation, here using the following adjustment variables: the biological variable of sex, age, race, and zip codes."), tags$br(),
          ...
          withMathJax("We will start by fitting a Binomial model \\(y^*_k\\), \\(y^*_k \\sim \\textrm{Binomial}(n_k, p_k)\\) with a logit function for \\(\\pi_k\\) with covariates including sex, age, race, zip codes, and time in weeks to allow time variation of prevalence over time in the multilevel model parameters."), tags$br(),
          ...
          withMathJax("Here \\(e_j\\) denotes the zip code level error term and follows a normal distribution or a spatial distribution to capture the geospatial dependency (e.g., the conditional autoregressive model)."), tags$br(),
          withMathJax("According to the test protocol, the sensitivity is unknown, and the specificity is around 100%. We solicit prior information from previous testing results and try different values of the hyper-parameters for sensitivity analysis."), tags$br(),
