    ...
    fluidRow(
      column(width = 6, offset = 3,
        tags$div(
          ...
          withMathJax("We will start by fitting a Binomial model for \\(y^*_k\\), \\(y^*_k \\sim \\textrm{Binomial}(n_k, p_k)\\) with a logit function for \\(\\pi_k\\) with covariates including sex, age, race, zip codes, and time in weeks to allow time variation of prevalence over time in the multilevel model parameters."), tags$br(),
          ...
          withMathJax("where \\(male_k\\) is an indicator for men; age[k], race[k], and zip[k] represent age, race, and zip code levels; and time[k] indices the time in weeks when the test result is collected for group \\(k\\). We include zip code level covariates in the model as geographic measures \\(\\vec{Z}^{zip}_{j}\\) listed above for zip code \\(j\\),"),
